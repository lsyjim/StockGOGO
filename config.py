...
class QuantConfig:
    ...
    R_TRACK_ENABLED = _env_bool('R_TRACK', True)   # BP12 驗收全過 → 預設開（見下方 commit 驗收摘要）；R_TRACK=0 可關
    ...
    # build_prompt_20：**單檔試算用的模擬資金，不是真實帳戶總額**。
    # 報告 03 區「建議部位」目前以單檔視角試算（current_positions 一律空清單），
    # 因此這個數字只影響「換算成幾股」，不影響部位百分比。
    # 多帳戶/真實持倉整合是下一輪任務，屆時應改讀實際帳戶資料。
    SIMULATED_CAPITAL = 1_000_000
