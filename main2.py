...
class RecommendationDialog:
    ...
    def __init__(self, parent, analysis_result):
        ...
        # 取得股票名稱（優先使用 name，否則使用 symbol）
        stock_name = analysis_result.get('name', analysis_result['symbol'])
        
        # v4.3：歷史分析時顯示日期
        if analysis_result.get('is_historical'):
            title = f"📊 {stock_name} 歷史分析報告 [{analysis_result.get('analysis_date', '')}]"
        else:
            title = f"📊 {stock_name} 完整量化分析報告 v4.3"
        ...
        # v4.3：歷史分析時標題也顯示日期
        if analysis_result.get('is_historical'):
            title_text = f"📈 {stock_name} 歷史分析 ({analysis_result.get('analysis_date', '')})"
        else:
            title_text = f"📈 {stock_name} 量化分析報告"
        ...
        # 定義標籤樣式
        self.text_widget.tag_config("title", font=("SimHei", 18, "bold"), foreground="#F39C12")
        self.text_widget.tag_config("subtitle", font=("SimHei", 13, "bold"), foreground="#5DADE2")
        self.text_widget.tag_config("positive", foreground="#2ECC71", font=("SimHei", 14, "bold"))
        self.text_widget.tag_config("negative", foreground="#E74C3C", font=("SimHei", 14, "bold"))
        self.text_widget.tag_config("neutral", foreground="#F1C40F", font=("SimHei", 14, "bold"))
        self.text_widget.tag_config("normal", font=("SimHei", 14), foreground="#D4D4D4")
        self.text_widget.tag_config("warning", foreground="#E67E22", font=("SimHei", 12, "italic"))
        self.text_widget.tag_config("info", foreground="#3498DB", font=("SimHei", 12))
        
        self._insert_analysis(analysis_result)
        self.text_widget.config(state=tk.DISABLED)
        
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(pady=(15, 0))
        ttk.Button(button_frame, text="關閉視窗", command=self.dialog.destroy, width=15).pack()
    
    def _insert_analysis(self, result):
        """插入分析內容"""
        ...
        if result.get('is_historical'):
            ...
            self.text_widget.insert(tk.END, "⏰ 歷史分析模式\n", "title")
            self.text_widget.insert(tk.END, f"   分析日期：{result.get('analysis_date', 'N/A')}\n", "warning")
            if result.get('requested_date') != result.get('analysis_date'):
                self.text_widget.insert(tk.END, f"   請求日期：{result.get('requested_date', 'N/A')}（已調整至最近交易日）\n", "info")
            self.text_widget.insert(tk.END, "   ⚠️ 部分數據為估算值，Forward PE 不可用\n", "info")
            self.text_widget.insert(tk.END, "═" * 70 + "\n\n", "warning")
        
        # v4.3.7 新增：第一行顯示目前股價和漲幅
        current_price = result.get('current_price', 0)
        price_change = result.get('price_change', 0)
        price_change_pct = result.get('price_change_pct', 0)
        stock_name = result.get('name', result.get('symbol', ''))
        
        # 添加大字體股價標籤
        self.text_widget.tag_config("price_header", font=("SimHei", 22, "bold"), foreground="#3498DB")
        self.text_widget.tag_config("price_up", font=("SimHei", 18, "bold"), foreground="#E74C3C")
        self.text_widget.tag_config("price_down", font=("SimHei", 18, "bold"), foreground="#2ECC71")
        self.text_widget.tag_config("price_flat", font=("SimHei", 18, "bold"), foreground="#F1C40F")
        
        if current_price > 0:
            self.text_widget.insert(tk.END, f"💰 目前股價：", "title")
            self.text_widget.insert(tk.END, f"${current_price:.2f}  ", "price_header")
            
            if price_change > 0:
                self.text_widget.insert(tk.END, f"▲ {price_change:.2f} (+{price_change_pct:.2f}%)\n", "price_up")
            elif price_change < 0:
                self.text_widget.insert(tk.END, f"▼ {abs(price_change):.2f} ({price_change_pct:.2f}%)\n", "price_down")
            else:
                self.text_widget.insert(tk.END, f"- 平盤\n", "price_flat")
        ...
        if isinstance(rec, dict):
            self.text_widget.insert(tk.END, "【📊 綜合分析結論】\n", "title")
            self.text_widget.insert(tk.END, "=" * 70 + "\n", "normal")
            ...
            downgraded = rec.get('downgraded', False)
            rr_ratio = rec.get('rr_ratio', 0)
            bias_20 = rec.get('bias_20', 0)
            
            # 判斷建議類型的顏色
            if any(x in overall for x in ["強烈建議買進", "建議買進", "可買進", "可進場"]):
                overall_tag = "positive"
            elif any(x in overall for x in ["減碼", "賣出", "出場", "殺低"]):
                overall_tag = "negative"
            elif any(x in overall for x in ["等拉回", "等回檔", "暫停", "觀望", "盈虧比"]):
                overall_tag = "warning"
            elif any(x in overall for x in ["持股", "續抱"]):
                overall_tag = "neutral"
            else:
                overall_tag = "neutral"
            
            self.text_widget.insert(tk.END, f"● 總體評分：{score} / 100\n", "subtitle")
            
            # v4.3：顯示觸發場景
            if scenario and scenario_name:
                self.text_widget.insert(tk.END, f"● 觸發場景：", "subtitle")
                scenario_tag = "positive" if scenario in ['B', 'B2'] else "negative" if scenario in ['D'] else "warning" if scenario in ['A', 'C'] else "neutral"
                self.text_widget.insert(tk.END, f"{scenario} - {scenario_name}\n", scenario_tag)
            
            self.text_widget.insert(tk.END, f"● 投資建議：", "subtitle")
            self.text_widget.insert(tk.END, f"{overall}\n", overall_tag)
            
            # 進場時機
            if action_timing:
                timing_tag = "positive" if any(x in action_timing for x in ["甜蜜", "可", "立即"]) else "warning" if any(x in action_timing for x in ["等", "過熱"]) else "neutral"
                self.text_widget.insert(tk.END, f"● 進場時機：", "subtitle")
                self.text_widget.insert(tk.END, f"{action_timing}\n", timing_tag)
            
            # v4.3：顯示關鍵指標
            if rr_ratio > 0:
                rr_tag = "positive" if rr_ratio >= 2 else "warning" if rr_ratio >= 1.5 else "negative"
                self.text_widget.insert(tk.END, f"● 盈虧比：", "subtitle")
                self.text_widget.insert(tk.END, f"{rr_ratio:.2f}\n", rr_tag)
            
            if bias_20 != 0:
                bias_tag = "negative" if bias_20 > 15 else "positive" if bias_20 < -10 else "normal"
                self.text_widget.insert(tk.END, f"● 20MA乖離：", "subtitle")
                self.text_widget.insert(tk.END, f"{bias_20:+.1f}%\n", bias_tag)
            
            # v4.3：顯示警示訊息
            if warning_message:
                self.text_widget.insert(tk.END, f"\n📝 ", "normal")
                self.text_widget.insert(tk.END, f"{warning_message}\n", "warning")
            
            # v4.3：顯示濾網降級信息
            if downgraded:
                self.text_widget.insert(tk.END, f"\n⚠️ 濾網降級：", "subtitle")
                original_rec = rec.get('original_recommendation', '')
                self.text_widget.insert(tk.END, f"原始建議「{original_rec}」已被降級\n", "warning")
                for f in rec.get('filters_applied', []):
                    self.text_widget.insert(tk.END, f"   • [{f.get('filter', '')}] {f.get('reason', '')} → {f.get('action', '')}\n", "info")
            
            # 信心度
            if confidence:
                conf_tag = "positive" if confidence == "High" else "warning" if confidence == "Low" else "neutral"
                self.text_widget.insert(tk.END, f"\n● 信心度：", "subtitle")
                self.text_widget.insert(tk.END, f"{confidence}\n\n", conf_tag)
            else:
                self.text_widget.insert(tk.END, "\n", "normal")
            
            # ============================================================
            # v4.4.3 新增：今日操作建議（明確的開盤決策指引）
            # ============================================================
            self.text_widget.insert(tk.END, "【🎯 今日操作建議】\n", "title")
            self.text_widget.insert(tk.END, "=" * 70 + "\n", "normal")
            
            # 生成今日操作建議
            today_action = self._generate_today_action_advice(result, rec)
            
            # 訊號狀態
            signal_status = today_action.get('signal_status', '無明確訊號')
            signal_tag = today_action.get('signal_tag', 'neutral')
            self.text_widget.insert(tk.END, f"📡 昨日訊號：", "subtitle")
            self.text_widget.insert(tk.END, f"{signal_status}\n", signal_tag)
            
            # 今日建議動作
            action_advice = today_action.get('action_advice', '觀望')
            action_tag = today_action.get('action_tag', 'neutral')
            self.text_widget.insert(tk.END, f"📌 今日建議：", "subtitle")
            self.text_widget.insert(tk.END, f"{action_advice}\n", action_tag)
            
            # 執行條件
            conditions = today_action.get('conditions', [])
            if conditions:
                self.text_widget.insert(tk.END, f"\n✅ 執行條件檢核：\n", "subtitle")
                for cond in conditions:
                    cond_tag = "positive" if cond.get('passed') else "negative"
                    cond_icon = "✓" if cond.get('passed') else "✗"
                    self.text_widget.insert(tk.END, f"   {cond_icon} {cond.get('name')}：", "normal")
                    self.text_widget.insert(tk.END, f"{cond.get('value')}\n", cond_tag)
            
            # 關鍵價位
            key_prices = today_action.get('key_prices', {})
            if key_prices:
                self.text_widget.insert(tk.END, f"\n💰 關鍵價位：\n", "subtitle")
                if key_prices.get('entry'):
                    self.text_widget.insert(tk.END, f"   進場參考：${key_prices['entry']:.2f}\n", "normal")
                if key_prices.get('stop_loss'):
                    self.text_widget.insert(tk.END, f"   停損價位：${key_prices['stop_loss']:.2f}\n", "negative")
                if key_prices.get('take_profit'):
                    self.text_widget.insert(tk.END, f"   停利目標：${key_prices['take_profit']:.2f}\n", "positive")
            
            # 最終結論
            final_verdict = today_action.get('final_verdict', '')
            if final_verdict:
                verdict_tag = today_action.get('verdict_tag', 'neutral')
                self.text_widget.insert(tk.END, f"\n🔔 結論：", "subtitle")
                self.text_widget.insert(tk.END, f"{final_verdict}\n\n", verdict_tag)
            
            # 操作建議
            self.text_widget.insert(tk.END, "【💡 分段操作建議】\n", "subtitle")
            for period, name in [('short_term', '短線 (1-5天)'), ('mid_term', '中線 (5-20天)'), ('long_term', '長線 (20天以上)')]:
                data = rec.get(period, {})
                action = data.get('action', 'N/A')
                reason = data.get('reason', 'N/A')
                
                # 更精確的顏色判斷
                if any(x in action for x in ["強烈", "買進", "進場", "看好", "偏多", "試單", "可買"]):
                    p_tag = "positive"
                elif any(x in action for x in ["賣出", "出場", "減碼", "偏空"]):
                    p_tag = "negative"
                elif any(x in action for x in ["等拉回", "等回檔", "過熱", "暫停", "搶反彈", "殺低"]):
                    p_tag = "warning"
                else:
                    p_tag = "neutral"
                
                self.text_widget.insert(tk.END, f"📍 {name}：", "normal")
                self.text_widget.insert(tk.END, f"{action}\n", p_tag)
                self.text_widget.insert(tk.END, f"   理由：{reason}\n\n", "normal")
        
        # 2. v4.3 新增：決策矩陣詳情
        dm = result.get('decision_matrix', {})
        if dm.get('available'):
            self.text_widget.insert(tk.END, "【🎯 多因子決策矩陣 v4.3】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            dv = dm.get('decision_vars', {})
            
            # 核心決策變數
            self.text_widget.insert(tk.END, "  📊 核心決策變數：\n", "subtitle")
            
            trend = dv.get('trend_status', 'N/A')
            trend_tag = "positive" if trend == "Bull" else "negative" if trend == "Bear" else "neutral"
            self.text_widget.insert(tk.END, f"     趨勢狀態：", "normal")
            trend_cn = "多頭" if trend == "Bull" else "空頭" if trend == "Bear" else "盤整"
            self.text_widget.insert(tk.END, f"{trend_cn}\n", trend_tag)
            
            bias_pos = dv.get('position_bias', 'N/A')
            bias_val = dv.get('bias_20', 0)
            bias_tag = "negative" if bias_pos == "High" else "positive" if bias_pos in ["Low", "DeepLow"] else "neutral"
            bias_cn = "過熱" if bias_pos == "High" else "深度超跌" if bias_pos == "DeepLow" else "超跌" if bias_pos == "Low" else "正常"
            self.text_widget.insert(tk.END, f"     乖離位置：", "normal")
            self.text_widget.insert(tk.END, f"{bias_cn}（{bias_val:+.1f}%）\n", bias_tag)
            
            rr = dv.get('rr_ratio', 0)
            rr_tag = "positive" if rr >= 2 else "warning" if rr >= 1.5 else "negative"
            self.text_widget.insert(tk.END, f"     風險回報比：", "normal")
            self.text_widget.insert(tk.END, f"{rr:.2f}\n", rr_tag)
            
            self.text_widget.insert(tk.END, f"     RSI：{dv.get('rsi', 'N/A'):.0f} | ADX：{dv.get('adx', 'N/A'):.0f}\n", "normal")
            
            # 量能異常
            vol_anomaly = dv.get('vol_anomaly', {})
            if vol_anomaly.get('has_anomaly'):
                self.text_widget.insert(tk.END, f"     ⚠️ 量能異常：", "warning")
                anomaly_msgs = [a['message'] for a in vol_anomaly.get('anomalies', [])]
                self.text_widget.insert(tk.END, f"{', '.join(anomaly_msgs)}\n", "warning")
            
            # 左側訊號
            if dv.get('left_buy_triggered'):
                self.text_widget.insert(tk.END, f"     🟢 左側買訊觸發\n", "positive")
            if dv.get('left_sell_triggered'):
                self.text_widget.insert(tk.END, f"     🔴 左側賣訊觸發\n", "negative")
            
            # v4.3.6 新增：區間操作詳細資訊（場景 E）
            range_info = dm.get('range_info', {})
            if range_info and dm.get('scenario') == 'E':
                self.text_widget.insert(tk.END, "\n  📦 區間操作詳細資訊：\n", "subtitle")
                
                box_top = range_info.get('box_top', 'N/A')
                box_bottom = range_info.get('box_bottom', 'N/A')
                position = range_info.get('position', 'N/A')
                position_pct = range_info.get('position_pct', 50)
                suggestion = range_info.get('suggestion', 'N/A')
                current = range_info.get('current_price', 'N/A')
                
                self.text_widget.insert(tk.END, f"     箱頂價格：", "normal")
                self.text_widget.insert(tk.END, f"${box_top}\n", "negative")
                self.text_widget.insert(tk.END, f"     箱底價格：", "normal")
                self.text_widget.insert(tk.END, f"${box_bottom}\n", "positive")
                self.text_widget.insert(tk.END, f"     目前股價：${current}\n", "normal")
                
                # 位置百分比視覺化
                self.text_widget.insert(tk.END, f"     區間位置：{position}（{position_pct:.0f}%）\n", "normal")
                
                # 根據位置給予顏色提示
                if position_pct <= 30:
                    position_tag = "positive"
                elif position_pct >= 70:
                    position_tag = "negative"
                else:
                    position_tag = "neutral"
                
                # 繪製簡易區間圖示
                bar_length = 20
                filled = int(position_pct / 100 * bar_length)
                bar = "▓" * filled + "░" * (bar_length - filled)
                self.text_widget.insert(tk.END, f"     位置圖示：[箱底 {bar} 箱頂]\n", position_tag)
                
                # 操作建議
                self.text_widget.insert(tk.END, f"     💡 操作建議：", "normal")
                if "買" in suggestion:
                    self.text_widget.insert(tk.END, f"{suggestion}\n", "positive")
                elif "賣" in suggestion:
                    self.text_widget.insert(tk.END, f"{suggestion}\n", "negative")
                else:
                    self.text_widget.insert(tk.END, f"{suggestion}\n", "warning")
        ...
        market_regime = result.get('market_regime', {})
        if market_regime.get('available'):
            self.text_widget.insert(tk.END, "【🌐 市場環境分析 v4.0】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            regime = market_regime.get('regime', 'N/A')
            trend = market_regime.get('trend_direction', 'N/A')
            adx = market_regime.get('adx', 'N/A')
            
            regime_tag = "positive" if "趨勢" in regime else "neutral"
            trend_tag = "positive" if trend == "多頭" else "negative" if trend == "空頭" else "neutral"
            
            self.text_widget.insert(tk.END, f"  大盤狀態：", "normal")
            self.text_widget.insert(tk.END, f"{regime}\n", regime_tag)
            self.text_widget.insert(tk.END, f"  趨勢方向：", "normal")
            self.text_widget.insert(tk.END, f"{trend}\n", trend_tag)
            self.text_widget.insert(tk.END, f"  ADX指標：{adx} ", "normal")
            
            if adx != 'N/A':
                if adx > 40:
                    self.text_widget.insert(tk.END, "(強趨勢)\n", "positive")
                elif adx > 25:
                    self.text_widget.insert(tk.END, "(趨勢中)\n", "neutral")
                else:
                    self.text_widget.insert(tk.END, "(震盪盤)\n", "warning")
            else:
                self.text_widget.insert(tk.END, "\n", "normal")
            
            self.text_widget.insert(tk.END, f"  近5日漲跌：{market_regime.get('pct_change_5d', 'N/A')}%\n", "normal")
            self.text_widget.insert(tk.END, f"  近20日漲跌：{market_regime.get('pct_change_20d', 'N/A')}%\n\n", "normal")
        
        # v4.1 新增：波段分析
        wave = result.get('wave_analysis', {})
        if wave.get('available'):
            self.text_widget.insert(tk.END, "【🌊 波段分析 v4.1（三盤突破/跌破）】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            # 波段狀態
            wave_status = wave.get('wave_status', 'N/A')
            if '🟢' in wave_status:
                status_tag = "positive"
            elif '🔴' in wave_status:
                status_tag = "negative"
            ...
            self.text_widget.insert(tk.END, f"  波段狀態：", "normal")
            self.text_widget.insert(tk.END, f"{wave_status}\n", status_tag)
            
            # 波段說明
            self.text_widget.insert(tk.END, f"  狀態說明：{wave.get('status_detail', 'N/A')}\n", "normal")
            
            # 進出場建議
            action = wave.get('action_advice', 'N/A')
            if '進場' in action or '介入' in action or '加碼' in action:
                action_tag = "positive"
            elif '出場' in action or '停損' in action:
                action_tag = "negative"
            else:
                action_tag = "neutral"
            self.text_widget.insert(tk.END, f"  操作建議：", "normal")
            ...
            # 環境指標
            self.text_widget.insert(tk.END, "  📊 環境指標：\n", "subtitle")
            
            ma55_trend = wave.get('ma55_trend', 'N/A')
            ma55_tag = "positive" if ma55_trend == '上揚' else "negative" if ma55_trend == '下彎' else "neutral"
            self.text_widget.insert(tk.END, f"     55MA趨勢：", "normal")
            self.text_widget.insert(tk.END, f"{ma55_trend}", ma55_tag)
            self.text_widget.insert(tk.END, f"（值={wave.get('ma55_value', 'N/A')}）\n", "normal")
            
            above_ma = wave.get('price_above_ma55', False)
            self.text_widget.insert(tk.END, f"     K線位置：", "normal")
            if above_ma:
                self.text_widget.insert(tk.END, f"55MA上方（+{wave.get('ma55_distance', 0)}%）\n", "positive")
            else:
                self.text_widget.insert(tk.END, f"55MA下方（{wave.get('ma55_distance', 0)}%）\n", "negative")
            
            # 量價共振
            sync = wave.get('volume_price_sync', {})
            sync_status = sync.get('sync', False)
            self.text_widget.insert(tk.END, f"     量價共振：", "normal")
            self.text_widget.insert(tk.END, f"{sync.get('description', 'N/A')}\n\n", "positive" if sync_status else "neutral")
            
            # 進場訊號
            breakout = wave.get('breakout_signal', {})
            if breakout.get('detected'):
                self.text_widget.insert(tk.END, "  🚀 進場訊號（三盤突破）：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     收盤價 {breakout.get('current_close')} > 前兩日高點 {breakout.get('breakout_level')}\n", "positive")
                vol_confirm = "✓ 量能確認" if breakout.get('volume_confirmed') else "量能未確認"
                above_open = "✓ 站穩開盤價" if breakout.get('above_open') else "跌破開盤價"
                self.text_widget.insert(tk.END, f"     {vol_confirm} | {above_open}\n", "normal")
                strength = breakout.get('strength', 'none')
                strength_text = "強勢突破" if strength == 'strong' else "一般突破" if strength == 'moderate' else ""
                if strength_text:
                    self.text_widget.insert(tk.END, f"     訊號強度：{strength_text}\n\n", "positive" if strength == 'strong' else "neutral")
            
            # 出場訊號
            breakdown = wave.get('breakdown_signal', {})
            if breakdown.get('detected'):
                self.text_widget.insert(tk.END, "  ⚠️ 出場訊號（三盤跌破）：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     收盤價 {breakdown.get('current_close')} < 前兩日低點 {breakdown.get('breakdown_level')}\n", "negative")
                self.text_widget.insert(tk.END, f"     跌破幅度：{breakdown.get('breakdown_pct')}%（{breakdown.get('severity', 'N/A')}）\n\n", "negative")
            
            # 爆量K線
            vol_bar = wave.get('volume_bar', {})
            if vol_bar.get('has_key_bar'):
                self.text_widget.insert(tk.END, "  📈 爆量K線守則：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     關鍵K線日期：{vol_bar.get('key_bar_date')}\n", "normal")
                self.text_widget.insert(tk.END, f"     關鍵低點：${vol_bar.get('key_bar_low')}（量比：{vol_bar.get('volume_ratio')}x）\n", "normal")
                warning = vol_bar.get('warning', '')
                warning_tag = "negative" if '跌破' in warning else "positive"
                self.text_widget.insert(tk.END, f"     狀態：{warning}\n\n", warning_tag)
        
        # v4.2 新增：均值回歸與乖離分析
        mr = result.get('mean_reversion', {})
        if mr.get('available'):
            self.text_widget.insert(tk.END, "【📐 均值回歸與乖離分析 v4.2】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            # 乖離率分析
            bias = mr.get('bias_analysis', {})
            self.text_widget.insert(tk.END, "  📊 乖離率分析：\n", "subtitle")
            
            bias_20 = bias.get('bias_20', 0)
            bias_60 = bias.get('bias_60', 0)
            
            # 20MA 乖離
            self.text_widget.insert(tk.END, f"     20MA乖離：", "normal")
            if bias_20 > QuantConfig.BIAS_OVERBOUGHT_THRESHOLD:
                self.text_widget.insert(tk.END, f"{bias_20:+.2f}% ⚠️ 嚴重過熱\n", "negative")
            elif bias_20 > 10:
                self.text_widget.insert(tk.END, f"{bias_20:+.2f}% 偏高\n", "warning")
            elif bias_20 < QuantConfig.BIAS_OVERSOLD_THRESHOLD:
                self.text_widget.insert(tk.END, f"{bias_20:+.2f}% ⚠️ 嚴重超跌\n", "positive")
            elif bias_20 < -5:
                self.text_widget.insert(tk.END, f"{bias_20:+.2f}% 偏低\n", "neutral")
            else:
                self.text_widget.insert(tk.END, f"{bias_20:+.2f}% 正常\n", "normal")
            
            # 60MA 乖離
            self.text_widget.insert(tk.END, f"     60MA乖離：", "normal")
            if abs(bias_60) > 15:
                bias_tag = "negative" if bias_60 > 0 else "positive"
                self.text_widget.insert(tk.END, f"{bias_60:+.2f}%\n", bias_tag)
            else:
                self.text_widget.insert(tk.END, f"{bias_60:+.2f}%\n", "normal")
            
            self.text_widget.insert(tk.END, f"     狀態：{bias.get('bias_20_status', 'N/A')}\n\n", "normal")
            ...
            if left_buy.get('triggered'):
                self.text_widget.insert(tk.END, "  🟢 左側買進訊號（超跌反彈）：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     {left_buy.get('message', '')}\n", "positive")
                for cond in left_buy.get('conditions_met', []):
                    self.text_widget.insert(tk.END, f"     ✓ {cond}\n", "normal")
                strength = left_buy.get('strength', 'none')
                self.text_widget.insert(tk.END, f"     訊號強度：{'強' if strength == 'strong' else '中等'}\n\n", 
                                       "positive" if strength == 'strong' else "neutral")
            
            if left_sell.get('triggered'):
                self.text_widget.insert(tk.END, "  🔴 左側賣出訊號（漲多預判拉回）：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     {left_sell.get('message', '')}\n", "negative")
                for reason in left_sell.get('trigger_reasons', []):
                    self.text_widget.insert(tk.END, f"     ⚠️ {reason}\n", "warning")
                strength = left_sell.get('strength', 'none')
                self.text_widget.insert(tk.END, f"     訊號強度：{'強' if strength == 'strong' else '中等'}\n\n",
                                       "negative" if strength == 'strong' else "warning")
            
            # 雙軌出場策略
            exit_strat = mr.get('exit_strategy', {})
            if exit_strat:
                self.text_widget.insert(tk.END, "  🚪 雙軌出場策略：\n", "subtitle")
                
                trend_exit = exit_strat.get('trend_exit', {})
                target_exit = exit_strat.get('target_exit', {})
                
                # 防守型出場
                self.text_widget.insert(tk.END, f"     {trend_exit.get('type', '🛡️ 防守型出場')}：", "normal")
                if trend_exit.get('triggered'):
                    self.text_widget.insert(tk.END, "已觸發\n", "negative")
                    for cond in trend_exit.get('conditions', []):
                        self.text_widget.insert(tk.END, f"        • {cond}\n", "normal")
                else:
                    self.text_widget.insert(tk.END, "未觸發（持續持有）\n", "positive")
                self.text_widget.insert(tk.END, f"        說明：{trend_exit.get('description', '')}\n", "info")
                
                # 積極型停利
                self.text_widget.insert(tk.END, f"     {target_exit.get('type', '💰 積極型停利')}：", "normal")
                if target_exit.get('triggered'):
                    self.text_widget.insert(tk.END, "建議執行\n", "warning")
                    for cond in target_exit.get('conditions', [])[:3]:  # 最多顯示3條
                        self.text_widget.insert(tk.END, f"        • {cond}\n", "normal")
                else:
                    self.text_widget.insert(tk.END, "未觸發\n", "positive")
                self.text_widget.insert(tk.END, f"        說明：{target_exit.get('description', '')}\n", "info")
                
                # 建議出場價位
                exit_prices = exit_strat.get('exit_prices', {})
                self.text_widget.insert(tk.END, f"     參考價位：趨勢停損=${exit_prices.get('trend_exit_level', 'N/A')} | ", "normal")
                self.text_widget.insert(tk.END, f"追蹤停損=${exit_prices.get('trailing_stop', 'N/A')}\n\n", "normal")
            
            # 操作建議總結
            op_advice = mr.get('operation_advice', {})
            if op_advice:
                self.text_widget.insert(tk.END, "  💡 操作建議總結：\n", "subtitle")
                self.text_widget.insert(tk.END, f"     趨勢狀態：{op_advice.get('trend_status', 'N/A')} | ", "normal")
                self.text_widget.insert(tk.END, f"乖離狀態：{op_advice.get('bias_status', 'N/A')}\n", "normal")
                
                combined = op_advice.get('combined_advice', '')
                if '過熱' in combined or '追高' in combined:
                    self.text_widget.insert(tk.END, f"     📌 {combined}\n", "warning")
                elif '超跌' in combined or '反彈' in combined:
                    self.text_widget.insert(tk.END, f"     📌 {combined}\n", "positive")
                else:
                    self.text_widget.insert(tk.END, f"     📌 {combined}\n", "normal")
                
                self.text_widget.insert(tk.END, f"     風險等級：{op_advice.get('risk_level', 'N/A')}\n", "normal")
                
                action = op_advice.get('suitable_action', '')
                if '停利' in action or '減碼' in action:
                    self.text_widget.insert(tk.END, f"     建議動作：{action}\n\n", "warning")
                elif '進場' in action or '持有' in action:
                    self.text_widget.insert(tk.END, f"     建議動作：{action}\n\n", "positive")
                else:
                    self.text_widget.insert(tk.END, f"     建議動作：{action}\n\n", "neutral")
        
        # 3. 各面向指標
        self.text_widget.insert(tk.END, "【📈 各面向指標明細】\n", "title")
        
        # v4.3 修改：顯示當前股價、來源、昨日股價、漲跌幅
        current_price = result.get('current_price', 'N/A')
        prev_close = result.get('prev_close', 'N/A')
        price_change = result.get('price_change', 0)
        price_change_pct = result.get('price_change_pct', 0)
        price_source = result.get('price_source', 'yfinance')
        
        # 即時股價來源標註
        source_text = "⚡即時" if price_source == 'yahoo_scrape' else "📊延遲"
        self.text_widget.insert(tk.END, f"當前報價：${current_price} ", "subtitle")
        self.text_widget.insert(tk.END, f"({source_text}) ", "info")
        
        # 漲跌顯示
        if price_change > 0:
            self.text_widget.insert(tk.END, f"▲{price_change} (+{price_change_pct}%)", "positive")
        elif price_change < 0:
            self.text_widget.insert(tk.END, f"▼{abs(price_change)} ({price_change_pct}%)", "negative")
        else:
            self.text_widget.insert(tk.END, f"－ (0%)", "neutral")
        
        self.text_widget.insert(tk.END, f" | 昨收：${prev_close}\n", "normal")
        
        if 'data_time' in result:
            self.text_widget.insert(tk.END, f"更新時間：{result['data_time']}\n\n", "info")
        
        # 技術面
        tech = result.get('technical', {})
        self.text_widget.insert(tk.END, "● 技術面分析\n", "subtitle")
        self.text_widget.insert(tk.END, f"  趨勢：{tech.get('trend', 'N/A')} ({tech.get('signal', 'N/A')})\n", "normal")
        self.text_widget.insert(tk.END, f"  RSI：{tech.get('rsi', 'N/A')} | ADX：{tech.get('adx', 'N/A')}\n", "normal")
        self.text_widget.insert(tk.END, f"  均線：MA5={tech.get('ma5', 'N/A')} | MA20={tech.get('ma20', 'N/A')}\n\n", "normal")
        
        # v4.0 改進：基本面（顯示 PE Band）
        fund = result.get('fundamental', {})
        self.text_widget.insert(tk.END, "● 基本面分析 (v4.0 PE Band)\n", "subtitle")
        
        fund_signal = fund.get('signal', 'N/A')
        fund_tag = "positive" if fund_signal == "偏多" else "negative" if fund_signal == "偏空" else "neutral"
        self.text_widget.insert(tk.END, f"  基本面訊號：", "normal")
        self.text_widget.insert(tk.END, f"{fund_signal}\n", fund_tag)
        
        # v4.4.2 修正：檢查 PE 是否為負值
        trailing_pe = fund.get('trailing_pe', 'N/A')
        forward_pe = fund.get('forward_pe', 'N/A')
        
        pe_negative_warning = False
        if isinstance(forward_pe, (int, float)) and forward_pe < 0:
            pe_negative_warning = True
        elif isinstance(trailing_pe, (int, float)) and trailing_pe < 0:
            pe_negative_warning = True
        
        self.text_widget.insert(tk.END, f"  當前PE：{trailing_pe} | 預估PE：{forward_pe}", "normal")
        
        # 如果 PE 為負值，顯示警告
        if pe_negative_warning:
            self.text_widget.insert(tk.END, " ⚠️ 公司虧損\n", "warning")
        else:
            self.text_widget.insert(tk.END, "\n", "normal")
        
        self.text_widget.insert(tk.END, f"  PE歷史百分位：{fund.get('pe_percentile', 'N/A')}% ", "normal")
        
        pe_band = fund.get('pe_band_signal', 'N/A')
        if "低檔" in str(pe_band):
            self.text_widget.insert(tk.END, f"({pe_band})\n", "positive")
        elif "高檔" in str(pe_band):
            self.text_widget.insert(tk.END, f"({pe_band})\n", "negative")
        else:
            self.text_widget.insert(tk.END, f"({pe_band})\n", "normal")
        
        self.text_widget.insert(tk.END, f"  評估理由：{fund.get('signal_reason', 'N/A')}\n\n", "info")
        
        # 籌碼面（v4.4.2 增強：顯示數值驅動的詳細資訊）
        chip = result.get('chip_flow', {})
        if chip.get('available'):
            self.text_widget.insert(tk.END, "● 籌碼面分析\n", "subtitle")
            
            # 資料來源
            data_source = chip.get('data_source', 'N/A')
            self.text_widget.insert(tk.END, f"  資料來源：{data_source}\n", "info")
            ...
            self.text_widget.insert(tk.END, f"  外資：", "normal")
            if foreign_net > 0:
                if abs(foreign_days) >= 2:
                    self.text_widget.insert(tk.END, f"🔴 {foreign_text}", "positive")
                    self.text_widget.insert(tk.END, f" （連{abs(foreign_days)}日買超）\n", "positive")
                else:
                    self.text_widget.insert(tk.END, f"🔴 {foreign_text}\n", "positive")
            elif foreign_net < 0:
                if abs(foreign_days) >= 2:
                    self.text_widget.insert(tk.END, f"🟢 {foreign_text}", "negative")
                    self.text_widget.insert(tk.END, f" （連{abs(foreign_days)}日賣超）\n", "negative")
                else:
                    self.text_widget.insert(tk.END, f"🟢 {foreign_text}\n", "negative")
            else:
                self.text_widget.insert(tk.END, f"⚪ {foreign_text}\n", "neutral")
            ...
            self.text_widget.insert(tk.END, f"  投信：", "normal")
            if trust_net > 0:
                if abs(trust_days) >= 2:
                    self.text_widget.insert(tk.END, f"🔴 {trust_text}", "positive")
                    self.text_widget.insert(tk.END, f" （連{abs(trust_days)}日買超）\n", "positive")
                else:
                    self.text_widget.insert(tk.END, f"🔴 {trust_text}\n", "positive")
            elif trust_net < 0:
                if abs(trust_days) >= 2:
                    self.text_widget.insert(tk.END, f"🟢 {trust_text}", "negative")
                    self.text_widget.insert(tk.END, f" （連{abs(trust_days)}日賣超）\n", "negative")
                else:
                    self.text_widget.insert(tk.END, f"🟢 {trust_text}\n", "negative")
            else:
                self.text_widget.insert(tk.END, f"⚪ {trust_text}\n", "neutral")
            ...
            # 自營商（如果有）
            dealer_text = chip.get('dealer', '')
            if dealer_text and dealer_text != "暫無數據":
                self.text_widget.insert(tk.END, f"  自營商：{dealer_text}\n", "normal")
            
            # v4.4.2 新增：同步買超/賣超信號判斷（數值驅動）
            is_sync_buy = (foreign_net > 0 and trust_net > 0 and 
                          abs(foreign_days) >= 2 and abs(trust_days) >= 2)
            is_sync_sell = (foreign_net < 0 and trust_net < 0 and 
                           abs(foreign_days) >= 2 and abs(trust_days) >= 2)
            ...
            if is_sync_buy:
                self.text_widget.insert(tk.END, f"  ⭐ 同步買超信號：", "subtitle")
                self.text_widget.insert(tk.END, f"外資投信同步連續買超，籌碼面強勢\n", "positive")
            elif is_sync_sell:
                self.text_widget.insert(tk.END, f"  ⚠️ 同步賣超風險：", "subtitle")
                self.text_widget.insert(tk.END, f"外資投信同步連續賣超，籌碼面轉弱\n", "negative")
            
            # 籌碼結論
            chip_signal = chip.get('signal', 'N/A')
            chip_tag = "positive" if "集中" in chip_signal or "偏多" in chip_signal else "negative" if "分散" in chip_signal or "偏空" in chip_signal else "neutral"
            self.text_widget.insert(tk.END, f"  籌碼結論：", "normal")
            self.text_widget.insert(tk.END, f"{chip_signal}\n", chip_tag)
            self.text_widget.insert(tk.END, f"  {chip.get('message', '')}\n\n", "info")
        
        # v4.0 新增：成交量異常偵測
        volume = result.get('volume_analysis', {})
        if volume.get('spike_detected') is not None:
            self.text_widget.insert(tk.END, "● 成交量分析 (Volume Spike v4.0)\n", "subtitle")
            
            if volume.get('spike_detected'):
                self.text_widget.insert(tk.END, f"  ⚡ 偵測到爆量！", "warning")
                self.text_widget.insert(tk.END, f" (當前量為均量的 {volume.get('volume_ratio', 1):.1f} 倍)\n", "normal")
                
                spike_action = volume.get('spike_action', 'N/A')
                spike_tag = "positive" if spike_action == "偏多" else "negative" if spike_action == "偏空" else "neutral"
                self.text_widget.insert(tk.END, f"  訊號解讀：", "normal")
                self.text_widget.insert(tk.END, f"{volume.get('spike_signal', 'N/A')}\n", spike_tag)
            else:
                self.text_widget.insert(tk.END, f"  成交量正常 (當前量為均量的 {volume.get('volume_ratio', 1):.1f} 倍)\n", "normal")
            
            self.text_widget.insert(tk.END, f"  近期量能趨勢：{volume.get('volume_trend', 'N/A')}\n\n", "normal")
        
        # v4.4.1 新增：量價分析情境庫
        vp = result.get('volume_price', {})
        if vp.get('available'):
            self.text_widget.insert(tk.END, "【📊 量價分析情境庫 v4.4】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            # 量價摘要
            self.text_widget.insert(tk.END, f"  📋 分析摘要：", "subtitle")
            summary = vp.get('summary', '無')
            vp_score = vp.get('vp_score', 0)
            if vp_score > 20:
                self.text_widget.insert(tk.END, f"{summary}\n", "positive")
            elif vp_score < -20:
                self.text_widget.insert(tk.END, f"{summary}\n", "negative")
            else:
                self.text_widget.insert(tk.END, f"{summary}\n", "neutral")
            
            self.text_widget.insert(tk.END, f"  量價分數：{vp_score:+d}\n", "normal")
            self.text_widget.insert(tk.END, f"  量比：{vp.get('vol_ratio', 1):.2f}x\n\n", "normal")
            
            # 觸發的情境
            signals = vp.get('signals', [])
            if signals:
                self.text_widget.insert(tk.END, f"  🎯 觸發情境（共{len(signals)}個）：\n", "subtitle")
                for sig in signals[:5]:  # 最多顯示5個
                    ...
                    self.text_widget.insert(tk.END, f"     {icon} ", "normal")
                    self.text_widget.insert(tk.END, f"{sig.get('name', 'N/A')}", tag)
                    self.text_widget.insert(tk.END, f" [{stars}]\n", "info")
                    self.text_widget.insert(tk.END, f"        證據：{sig.get('evidence', 'N/A')}\n", "info")
                    self.text_widget.insert(tk.END, f"        建議：", "normal")
                    self.text_widget.insert(tk.END, f"{sig.get('decision_hint', 'N/A')}\n", tag)
            ...
            if decision_hint:
                self.text_widget.insert(tk.END, f"  💡 量價決策提示：", "subtitle")
                if 'BUY' in decision_hint:
                    self.text_widget.insert(tk.END, f"{decision_hint}\n", "positive")
                elif 'SELL' in decision_hint:
                    self.text_widget.insert(tk.END, f"{decision_hint}\n", "negative")
                else:
                    self.text_widget.insert(tk.END, f"{decision_hint}\n", "warning")
            
            if risk_notes:
                self.text_widget.insert(tk.END, f"  ⚠️ 風險提示：{risk_notes}\n", "warning")
        ...
        if risk_mgr.get('available'):
            self.text_widget.insert(tk.END, "【🛡️ 風險管理分析 v4.4】\n", "title")
            self.text_widget.insert(tk.END, "-" * 70 + "\n", "normal")
            
            self.text_widget.insert(tk.END, f"  進場價：${risk_mgr.get('entry_price', 0):.2f}\n", "normal")
            self.text_widget.insert(tk.END, f"  建議停損：", "normal")
            self.text_widget.insert(tk.END, f"${risk_mgr.get('stop_loss', 0):.2f}\n", "negative")
            self.text_widget.insert(tk.END, f"  建議停利：", "normal")
            self.text_widget.insert(tk.END, f"${risk_mgr.get('take_profit', 0):.2f}\n", "positive")
            self.text_widget.insert(tk.END, f"  ATR：{risk_mgr.get('atr', 0):.2f} ({risk_mgr.get('atr_pct', 0):.1f}%)\n", "normal")
            
            rr = risk_mgr.get('rr_ratio', 0)
            rr_tag = "positive" if rr >= 2 else "warning" if rr >= 1.5 else "negative"
            self.text_widget.insert(tk.END, f"  風險回報比：", "normal")
            self.text_widget.insert(tk.END, f"{rr:.2f}\n", rr_tag)
            ...
            self.text_widget.insert(tk.END, f"  流動性：20日均量 {liquidity.get('avg_volume_lots', 0):.0f} 張", "normal")
            if liquidity.get('tradable'):
                self.text_widget.insert(tk.END, " ✅\n", "positive")
            else:
                self.text_widget.insert(tk.END, " ⚠️ 流動性不足\n", "warning")
            
            # 跳空風險
            gap_risk = risk_mgr.get('gap_risk', {})
            if gap_risk.get('gap_risk_flag'):
                self.text_widget.insert(tk.END, f"  ⚠️ 跳空風險：{gap_risk.get('reason', '')}\n", "warning")
            
            # 建議倉位
            pos_size = risk_mgr.get('position_size_suggestion', 0)
            pos_value = risk_mgr.get('position_value', 0)
            if pos_size > 0:
                self.text_widget.insert(tk.END, f"  建議倉位：{pos_size:,} 股（約 ${pos_value:,.0f}）\n", "normal")
            
            # 可交易性
            tradable = risk_mgr.get('tradable', True)
            tradable_reason = risk_mgr.get('tradable_reason', '')
            self.text_widget.insert(tk.END, f"  可交易性：", "normal")
            if tradable:
                self.text_widget.insert(tk.END, f"符合條件 ✅\n", "positive")
            else:
                self.text_widget.insert(tk.END, f"{tradable_reason}\n", "negative")
            
            self.text_widget.insert(tk.END, "\n", "normal")
        
        # v4.0 改進：風險指標（顯示 Beta）
        risk = result.get('risk_metrics', {})
        self.text_widget.insert(tk.END, "● 風險指標 (v4.0 長期數據)\n", "subtitle")
        self.text_widget.insert(tk.END, f"  年化波動率：{risk.get('volatility', 'N/A')}% ({risk.get('vol_level', 'N/A')})\n", "normal")
        self.text_widget.insert(tk.END, f"  VaR(95%)：{risk.get('var_95', 'N/A')}% | VaR(99%)：{risk.get('var_99', 'N/A')}%\n", "normal")
        self.text_widget.insert(tk.END, f"  最大回撤：{risk.get('max_drawdown', 'N/A')}%\n", "normal")
        
        beta = risk.get('beta', 'N/A')
        beta_type = risk.get('beta_type', 'N/A')
        self.text_widget.insert(tk.END, f"  Beta係數：{beta} ", "normal")
        if beta_type != 'N/A':
            beta_tag = "positive" if "防禦" in beta_type else "negative" if "攻擊" in beta_type else "neutral"
            self.text_widget.insert(tk.END, f"({beta_type})\n", beta_tag)
        else:
            self.text_widget.insert(tk.END, "\n", "normal")
        
        self.text_widget.insert(tk.END, f"  數據期間：{risk.get('data_period', 'N/A')}\n\n", "info")
        
        # 停損停利
        sr = result.get('support_resistance', {})
        self.text_widget.insert(tk.END, "● 停損停利建議\n", "subtitle")
        self.text_widget.insert(tk.END, f"  第一壓力位：${sr.get('resistance1', 'N/A')}\n", "normal")
        self.text_widget.insert(tk.END, f"  第一支撐位：${sr.get('support1', 'N/A')}\n", "normal")
        self.text_widget.insert(tk.END, f"  建議停利：", "normal")
        self.text_widget.insert(tk.END, f"${sr.get('take_profit', 'N/A')}\n", "positive")
        self.text_widget.insert(tk.END, f"  建議停損：", "normal")
        self.text_widget.insert(tk.END, f"${sr.get('stop_loss', 'N/A')}\n", "negative")
        self.text_widget.insert(tk.END, f"  風險回報比：{sr.get('risk_reward', 'N/A')}:1\n\n", "normal")
        
        # 推薦策略
        best = result.get('best_strategy', '觀望')
        self.text_widget.insert(tk.END, "【🎯 推薦策略】\n", "title")
        self.text_widget.insert(tk.END, f"系統推薦：", "normal")
        self.text_widget.insert(tk.END, f"{best}\n", "positive")
        self.text_widget.insert(tk.END, "理由：結合當前指標強度、歷史回測績效、穩定性(Sharpe)及市場環境之綜合最優選。\n\n", "info")
        
        # v4.3 新增：歷史分析的未來驗證區塊
        if result.get('is_historical'):
            future_val = result.get('future_validation', {})
            if future_val.get('available'):
                self.text_widget.insert(tk.END, "【🔮 策略驗證（分析日期之後的實際走勢）】\n", "title")
                self.text_widget.insert(tk.END, "=" * 70 + "\n", "normal")
                
                analysis_price = future_val.get('analysis_price', 0)
                self.text_widget.insert(tk.END, f"  📍 分析日股價：${analysis_price}\n", "subtitle")
                
                # 5天後
                if '5d_change' in future_val:
                    change_5d = future_val['5d_change']
                    tag_5d = "positive" if change_5d > 0 else "negative" if change_5d < 0 else "neutral"
                    self.text_widget.insert(tk.END, f"  📈 5天後：${future_val['5d_price']} ", "normal")
                    self.text_widget.insert(tk.END, f"({change_5d:+.2f}%)\n", tag_5d)
                
                # 10天後
                if '10d_change' in future_val:
                    change_10d = future_val['10d_change']
                    tag_10d = "positive" if change_10d > 0 else "negative" if change_10d < 0 else "neutral"
                    self.text_widget.insert(tk.END, f"  📈 10天後：${future_val['10d_price']} ", "normal")
                    self.text_widget.insert(tk.END, f"({change_10d:+.2f}%)\n", tag_10d)
                
                # 20天後
                if '20d_change' in future_val:
                    change_20d = future_val['20d_change']
                    tag_20d = "positive" if change_20d > 0 else "negative" if change_20d < 0 else "neutral"
                    self.text_widget.insert(tk.END, f"  📈 20天後：${future_val['20d_price']} ", "normal")
                    self.text_widget.insert(tk.END, f"({change_20d:+.2f}%)\n", tag_20d)
                
                # 區間最高/最低
                self.text_widget.insert(tk.END, "\n  📊 區間統計：\n", "subtitle")
                max_change = future_val.get('max_change', 0)
                min_change = future_val.get('min_change', 0)
                self.text_widget.insert(tk.END, f"     最高點：${future_val.get('max_price', 'N/A')} ", "normal")
                self.text_widget.insert(tk.END, f"({max_change:+.2f}%)\n", "positive")
                self.text_widget.insert(tk.END, f"     最低點：${future_val.get('min_price', 'N/A')} ", "normal")
                self.text_widget.insert(tk.END, f"({min_change:+.2f}%)\n", "negative")
                
                # 當前價格
                current_change = future_val.get('current_change', 0)
                tag_current = "positive" if current_change > 0 else "negative" if current_change < 0 else "neutral"
                days_elapsed = future_val.get('days_elapsed', 0)
                self.text_widget.insert(tk.END, f"\n  📍 目前（{days_elapsed}天後）：${future_val.get('current_price', 'N/A')} ", "normal")
                self.text_widget.insert(tk.END, f"({current_change:+.2f}%)\n", tag_current)
                
                # 策略驗證結論
                self.text_widget.insert(tk.END, "\n  🎯 驗證結論：", "subtitle")
                rec_overall = result.get('recommendation', {}).get('overall', '')
                if '買進' in rec_overall or '進場' in rec_overall:
                    if current_change > 5:
                        self.text_widget.insert(tk.END, "✅ 買進建議正確，獲利 ", "positive")
                        self.text_widget.insert(tk.END, f"{current_change:+.2f}%\n\n", "positive")
                    elif current_change > 0:
                        self.text_widget.insert(tk.END, "✅ 買進方向正確，小幅獲利 ", "positive")
                        self.text_widget.insert(tk.END, f"{current_change:+.2f}%\n\n", "neutral")
                    else:
                        self.text_widget.insert(tk.END, "❌ 買進建議失準，虧損 ", "negative")
                        self.text_widget.insert(tk.END, f"{current_change:+.2f}%\n\n", "negative")
                elif '賣出' in rec_overall or '減碼' in rec_overall:
                    if current_change < -5:
                        self.text_widget.insert(tk.END, "✅ 賣出建議正確，避開下跌 ", "positive")
                        self.text_widget.insert(tk.END, f"{current_change:+.2f}%\n\n", "positive")
                    elif current_change < 0:
                        self.text_widget.insert(tk.END, "✅ 賣出方向正確 ", "positive")
                        self.text_widget.insert(tk.END, f"（股價下跌 {current_change:+.2f}%）\n\n", "neutral")
                    else:
                        self.text_widget.insert(tk.END, "❌ 賣出建議失準，錯失漲幅 ", "warning")
                        self.text_widget.insert(tk.END, f"{current_change:+.2f}%\n\n", "warning")
                elif '觀望' in rec_overall or '等' in rec_overall:
                    self.text_widget.insert(tk.END, f"觀望建議，後續走勢 {current_change:+.2f}%\n\n", "neutral")
                else:
                    self.text_widget.insert(tk.END, f"後續走勢 {current_change:+.2f}%\n\n", "neutral")
            else:
                self.text_widget.insert(tk.END, "【🔮 策略驗證】\n", "title")
                self.text_widget.insert(tk.END, f"  {future_val.get('message', '無法取得後續數據')}\n\n", "info")
        
        # 免責聲明
        self.text_widget.insert(tk.END, "=" * 70 + "\n", "normal")
        self.text_widget.insert(tk.END, "⚠️ 提醒：投資有風險，本分析僅供參考，請嚴格執行個人止損計劃。\n", "warning")
        self.text_widget.insert(tk.END, "📊 v4.3 新特性：多因子決策矩陣、五大場景判斷、風險濾網、一致性建議輸出\n", "info")
        if result.get('is_historical'):
            self.text_widget.insert(tk.END, "📅 歷史分析模式：可驗證策略準確度，未來驗證區塊顯示實際走勢\n", "info")
    
    def _generate_today_action_advice(self, result, rec):
        """
        v4.4.4 重構：生成今日操作建議（引入否決權機制）
        ...
        try:
            ...
            # 技術面
            tech = result.get('technical', {})
            tech_signal = tech.get('signal', '中性')
            rsi = tech.get('rsi', 50)
            
            # 基本面（v4.4.4 新增：取得 PE 狀態）
            fund = result.get('fundamental', {})
            forward_pe = fund.get('forward_pe', 'N/A')
            pe_is_negative = False
            if forward_pe not in ['N/A', None] and isinstance(forward_pe, (int, float)):
                pe_is_negative = forward_pe < 0
            
            # 籌碼面
            chip = result.get('chip_flow', {})
            chip_signal = chip.get('signal', '中性') if chip.get('available') else '中性'
            
            # 風險管理
            risk = result.get('risk_management', {})
            stop_loss = risk.get('stop_loss', 0) if risk.get('available') else 0
            take_profit = risk.get('take_profit', 0) if risk.get('available') else 0
            entry_price = risk.get('entry_price', 0) if risk.get('available') else result.get('current_price', 0)
            tradable = risk.get('tradable', True) if risk.get('available') else True
            ...
            if scenario in ['B', 'B2']:
                ...
                buy_reasons.append(f"場景{scenario}（{scenario_name}）")
            
            # 左側買訊（超跌）
            if bias_20 < -10 and rsi < 30:
                buy_signal_triggered = True
                buy_reasons.append(f"超跌反彈（乖離{bias_20:.1f}%，RSI={rsi:.0f}）")
            
            # 技術+籌碼雙多
            if tech_signal == "偏多" and chip_signal in ["籌碼集中", "籌碼偏多"]:
                buy_signal_triggered = True
                buy_reasons.append("技術面+籌碼面雙多")
            
            # 帶量突破
            for sig in vp_signals:
                if sig.get('code') == 'VP05':  # 帶量突破
                    buy_signal_triggered = True
                    buy_reasons.append("帶量突破")
            ...
            if scenario == 'D':
                ...
                sell_reasons.append(f"場景D（{scenario_name}）")
            
            # 左側賣訊（過熱）
            if bias_20 > 15 and rsi > 75:
                sell_signal_triggered = True
                sell_reasons.append(f"過熱回檔（乖離{bias_20:.1f}%，RSI={rsi:.0f}）")
            
            # 放量跌破
            for sig in vp_signals:
                if sig.get('code') in ['VP03', 'VP08', 'VP12']:
                    sell_signal_triggered = True
                    sell_reasons.append(sig.get('name', '空方訊號'))
            
            # ============================================================
            # v4.4.4 核心修正：阻擋條件檢查（否決權機制）
            # ============================================================
            blockers = []
            ...
            if scenario == 'A':
                blockers.append({
                    'type': 'OVERHEATED',
                    'reason': f"場景A（{scenario_name}）- 短線嚴重過熱",
                    'action': "禁止追價，持股續抱或停利"
                })
            
            # 阻擋條件 2：盈虧比不佳（RR < 1.5 或無效）
            rr_valid = isinstance(rr_ratio, (int, float)) and rr_ratio > 0
            if not rr_valid or rr_ratio < QuantConfig.MIN_RR_RATIO:
                # v4.4.4 Fix: 修正 f-string 格式化語法
                rr_display = f"{rr_ratio:.2f}" if rr_valid else "N/A"
                blockers.append({
                    'type': 'BAD_RR',
                    'reason': f"盈虧比不佳（RR={rr_display}，需≥{QuantConfig.MIN_RR_RATIO}）",
                    'action': "風險過高，建議觀望"
                })
            
            # 阻擋條件 3：基本面風險（虧損 + 高乖離）
            if pe_is_negative and bias_20 > 10:
                blockers.append({
                    'type': 'FUNDAMENTAL_RISK',
                    'reason': f"公司虧損(PE<0)且乖離{bias_20:.1f}%偏高",
                    'action': "投機股風險，謹慎操作"
                })
            ...
            # 生成訊號狀態（純粹顯示訊號，不考慮阻擋）
            # ============================================================
            if buy_signal_triggered and not sell_signal_triggered:
                advice['signal_status'] = f"🟢 買進訊號觸發：{', '.join(buy_reasons[:2])}"
                advice['signal_tag'] = 'positive'
            elif sell_signal_triggered and not buy_signal_triggered:
                advice['signal_status'] = f"🔴 賣出訊號觸發：{', '.join(sell_reasons[:2])}"
                advice['signal_tag'] = 'negative'
            elif buy_signal_triggered and sell_signal_triggered:
                advice['signal_status'] = "⚠️ 訊號矛盾，多空交戰"
                advice['signal_tag'] = 'warning'
            else:
                advice['signal_status'] = "⚪ 無明確買賣訊號"
                advice['signal_tag'] = 'neutral'
            
            # ============================================================
            # v4.4.4 核心：生成今日操作建議（嚴格優先級順序）
            # 優先級：賣出 > 阻擋(否決權) > 買進 > 無訊號
            # ============================================================
            
            # 最高優先級：賣出訊號
            if sell_signal_triggered:
                if confidence == 'High':
                    advice['action_advice'] = "🔴 今日應賣出或減碼（賣出訊號明確）"
                    advice['action_tag'] = 'negative'
                else:
                    advice['action_advice'] = "🔴 建議減碼觀望（賣出訊號觸發）"
                    advice['action_tag'] = 'negative'
            
            # 次高優先級：阻擋條件（即使有買進訊號也要被否決）
            elif has_blockers:
                # 找出最嚴重的阻擋條件
                if any(b['type'] == 'OVERHEATED' for b in blockers):
                    advice['action_advice'] = "⚠️ 短線嚴重過熱，禁止追價，建議持股續抱或停利"
                    advice['action_tag'] = 'warning'
                elif any(b['type'] == 'BAD_RR' for b in blockers):
                    if buy_signal_triggered:
                        advice['action_advice'] = "⏸️ 買進訊號觸發，但盈虧比不佳（風險過高），建議觀望"
                    else:
                        advice['action_advice'] = "⏸️ 盈虧比不佳，暫不適合進場"
                    advice['action_tag'] = 'warning'
                elif any(b['type'] == 'FUNDAMENTAL_RISK' for b in blockers):
                    advice['action_advice'] = "⚠️ 投機股風險警示：公司虧損且乖離偏高，謹慎操作"
                    advice['action_tag'] = 'warning'
                elif any(b['type'] == 'RSI_OVERBOUGHT' for b in blockers):
                    advice['action_advice'] = "⏸️ RSI 嚴重超買，等拉回再考慮"
                    advice['action_tag'] = 'warning'
                else:
                    advice['action_advice'] = "⏸️ 存在風險阻擋條件，建議觀望"
                    advice['action_tag'] = 'warning'
            
            # 第三優先級：買進訊號（無阻擋時才執行）
            elif buy_signal_triggered:
                if confidence == 'High' and tradable:
                    advice['action_advice'] = "✅ 今日可買進（訊號明確，條件齊全）"
                    advice['action_tag'] = 'positive'
                elif confidence == 'High':
                    advice['action_advice'] = "✅ 今日可買進（注意流動性）"
                    advice['action_tag'] = 'positive'
                elif confidence == 'Medium' and tradable:
                    advice['action_advice'] = "✅ 訊號觸發，可適量買進"
                    advice['action_tag'] = 'positive'
                else:
                    advice['action_advice'] = "⏸️ 訊號偏多但信心度不足，可小量試單"
                    advice['action_tag'] = 'warning'
            
            # 最低優先級：無明確訊號
            else:
                if scenario == 'C':
                    advice['action_advice'] = "⏸️ 不宜殺低，等反彈再處理"
                    advice['action_tag'] = 'warning'
                elif scenario == 'E':
                    advice['action_advice'] = "📊 區間操作：靠近箱底買，靠近箱頂賣"
                    advice['action_tag'] = 'neutral'
                else:
                    advice['action_advice'] = "⏸️ 觀望等待，無明確進場訊號"
                    advice['action_tag'] = 'neutral'
            
            # ============================================================
            # 執行條件檢核（v4.4.4 新增阻擋條件顯示）
            # ============================================================
            # v4.4.4 Fix: 預先計算 rr_display 避免 f-string 解析問題
            rr_display_value = f"{rr_ratio:.2f}" if rr_valid else "N/A"
            
            advice['conditions'] = [
                {
                    'name': '訊號觸發',
                    'value': '是' if buy_signal_triggered else '否',
                    'passed': buy_signal_triggered
                },
                {
                    'name': '盈虧比 ≥ 1.5',
                    'value': rr_display_value,
                    'passed': rr_valid and rr_ratio >= QuantConfig.MIN_RR_RATIO
                },
                {
                    'name': '非過熱場景',
                    'value': '是' if scenario != 'A' else '否（場景A）',
                    'passed': scenario != 'A'
                },
                {
                    'name': '信心度',
            ...
            # ============================================================
            if entry_price > 0:
                advice['key_prices']['entry'] = entry_price
            if stop_loss > 0:
                advice['key_prices']['stop_loss'] = stop_loss
            if take_profit > 0:
                advice['key_prices']['take_profit'] = take_profit
            
            # ============================================================
            # 最終結論（v4.4.4 修正：考慮阻擋條件）
            # ============================================================
            conditions_passed = sum(1 for c in advice['conditions'] if c['passed'])
            
            if sell_signal_triggered:
                advice['final_verdict'] = "🔴 賣出訊號觸發，持股者應執行停利/停損"
                advice['verdict_tag'] = 'negative'
            elif has_blockers:
                # 有阻擋條件時，即使買訊觸發也不建議執行
                blocker_names = [b['type'] for b in blockers]
                if 'OVERHEATED' in blocker_names:
                    advice['final_verdict'] = "⚠️ 過熱警示：短線追價風險極高，建議持股觀望或停利"
                    advice['verdict_tag'] = 'warning'
                else:
                    advice['final_verdict'] = f"⚠️ 存在 {len(blockers)} 個風險阻擋，暫不建議進場"
                    advice['verdict_tag'] = 'warning'
            elif buy_signal_triggered and conditions_passed >= 4:
                advice['final_verdict'] = "🟢 條件齊全，今日開盤可執行買進"
                advice['verdict_tag'] = 'positive'
            elif buy_signal_triggered and conditions_passed >= 3:
                advice['final_verdict'] = "🟡 訊號觸發但條件未全，建議小量試單"
                advice['verdict_tag'] = 'warning'
            else:
                advice['final_verdict'] = "⚪ 今日無明確操作機會，持續觀察"
                advice['verdict_tag'] = 'neutral'
            
        except Exception as e:
            print(f"生成今日操作建議錯誤: {e}")
            import traceback
            traceback.print_exc()
            advice['final_verdict'] = f"分析錯誤: {str(e)}"
