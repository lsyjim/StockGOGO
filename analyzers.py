...
class DecisionMatrix:
    ...
    # 評分區間定義
    SCORE_ZONE_HIGH = 70      # 高分區：分數 >= 70
    SCORE_ZONE_MID_LOW = 40   # 低分區：分數 <= 40
    # 中分區：40 < 分數 < 70
    ...
    INVESTMENT_SCENARIOS = {
        ...
        'I': {
            ...
            'long_zone': 'Mid',
            'description': (
                '睡獅乍醒。基本面平平，但短線突然爆量轉強。可能是「轉機股」或「消息面驅動」。'
                '建議列入觀察名單，若能站穩關鍵壓力區（如季線）三天以上，可小量試單。'
            ),
            'risk_level': 'Medium',
            'position_advice': '小量試單，不超過總資金 20%',
            'stop_loss_advice': '跌破突破點即停損',
            'emoji': '👀'
        }
    }
